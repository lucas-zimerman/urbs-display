...
// Por hora aceita qualquer guid; representa a parada onde o painel está.
const idParadaAtual = "00000000-0000-0000-0000-000000000000";
...
function ligar() {
  ...
  setTimeout(() => {
    // Só liga os timers de desconto/scroll depois que os dados chegam:
    // antes disso proximasLinhas está vazio, e um tick de AtualizaTela
    // limparia a mensagem inicial sem ter nada pra desenhar no lugar.
    CarregarProximasLinhas().then(() => {
      setInterval(DescontaTempoChegada, INTERVALO_DESCONTO_MS);
      setInterval(AvancaScroll, INTERVALO_SCROLL_MS);
    });
...
// snd/602.opus pra linha 602, snd/others.opus pra qualquer outra.
function tocarSomChegada(numeroLinha) {
  if (!somHabilitado) return;

  const numero = numeroLinha.padStart(3, "0").slice(-3);
  const src = numero === "502" ? "snd/502.opus" : "snd/others.opus";
...
// piscando 5x, 2) as próximas paradas da rota com tempo e nome escorregando
// juntos (3x mais rápido que o scroll normal). No fim, volta pro quadro normal.
async function mostrarAnuncioChegada(linhaChegada) {
  ...
  const rota = await GetLinhaRota(linhaChegada.numeroLinha);
  if (rota) {
    await mostrarScrollParadas(cabecalho, rota);
  }
...
async function mostrarScrollParadas(cabecalho, rota) {
